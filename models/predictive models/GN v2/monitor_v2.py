...
class GraphNetsMonitor:
    ...
    def train(self):
      ...
      dicts_in_static, dicts_in_dynamic, dicts_out_static, dicts_out_dynamic = sample_trajectories_train(self.env, self.nb_trajectories, self.m, self.l, self.g, self.build_dics_function)
      print("Trajectories created, building training data")
      
      X_train = utils_tf.data_dicts_to_graphs_tuple(dicts_in_dynamic)
      Y_train = utils_tf.data_dicts_to_graphs_tuple(dicts_out_dynamic[self.T-1:])
      
      #print(utils_tf.graphs_tuple_to_data_dicts(Y_train))

      print("Training data built, training the model")
      self.model = RecurrentModelInference(name=self.name,
      ...
      for node in range(n_nodes):
          ...
          print(coords_ref.shape, coords_output.shape)
          ...
          for i in range(n_feats):
              ...
              plt.plot(range(min(coords_ref.shape[0], self.plot_threshold)), coords_ref[:self.plot_threshold,i], color=tuple(color), label='Ground Truth Trajectory')
              plt.plot(range(min(coords_output.shape[0], self.plot_threshold)), coords_output[:self.plot_threshold,i], color=tuple(color), label='Predicted with GNN', linestyle='dashed')
              plt.title('Train Forward - Ground Truth vs Prediction - component ' + str(i) + ', node ' + str(node))
              plt.legend()
    ...
    def test(self):
        ...
        dicts_in_static_test, dicts_in_dynamic_test, dicts_out_static_test, dicts_out_dynamic_test = sample_trajectories_test(self.env, self.nb_trajectories, self.m, self.l, self.g, self.build_dics_function)
            
        n_graphs = self.T
        n_points = len(dicts_out_static_test)
        ...
        loss_test = tf.reduce_mean((Y_test.nodes - output_test_nodes)**2) + float(self.nb_features_globals)/self.nb_features_nodes*tf.reduce_mean((Y_test.globals - output_test_globals)**2)
        ...
        for node in range(n_nodes):
            ...
            coords_output = self.sess.run(output_test_nodes)[node::n_nodes, :]
